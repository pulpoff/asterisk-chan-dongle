...
#include <asterisk/module.h>			/* ast_module_ref() ast_module_info */
...
#include <asterisk/format_cache.h>		/* ast_format_slin */
#include <asterisk/format_cap.h>		/* ast_format_cap */
...
static struct ast_format_cap *channel_tech_caps;
...
EXPORT_DEF int channels_loop(struct pvt * pvt, const struct ast_channel * requestor)
{
	...
	if(requestor && ast_channel_tech(requestor) == &channel_tech && ast_channel_tech_pvt(requestor) && ((struct cpvt*)ast_channel_tech_pvt(requestor))->pvt == pvt)
		return 1;
	return 0;
}
...
#define subclass_codec		subclass.format
...
static struct ast_frame* channel_read (struct ast_channel* channel)
{
	...
	else
	{
		...
		cpvt->a_read_frame.subclass_codec = ast_format_slin;
...
static int channel_write (struct ast_channel* channel, struct ast_frame* f)
{
	...
	if (f->frametype != AST_FRAME_VOICE || ast_format_cmp(f->subclass_codec, ast_format_slin) != AST_FORMAT_CMP_EQUAL)
	{
		return 0;
	}
	...
	{
		struct ast_channel* bridged = ast_channel_bridge_peer(channel);
...
EXPORT_DEF struct ast_channel* new_channel (struct pvt* pvt, int ast_state, const char* cid_num, int call_idx, unsigned dir, call_state_t state, const char * dnid, const struct ast_assigned_ids *assignedids, const struct ast_channel * requestor)
{
	struct ast_channel* channel;
	struct cpvt * cpvt;
...
EXPORT_DEF void start_local_channel (struct pvt* pvt, const char* exten, const char* number, channel_var_t* vars)
{
	...
	cap = ast_format_cap_alloc(AST_FORMAT_CAP_FLAG_DEFAULT);
	if (!cap) {
		ast_log (LOG_ERROR, "[%s] Unable to allocate format cap\n", PVT_ID(pvt));
		return;
	}
	ast_format_cap_append_by_type(cap, AST_MEDIA_TYPE_AUDIO);

	channel = ast_request ("Local", cap, NULL, NULL, channel_name, &cause);
	ao2_ref(cap, -1);
...
{
	ao2_cleanup(channel_tech_caps);
	channel_tech_caps = NULL;
}

EXPORT_DEF struct ast_format_cap *channel_tech_caps_get(void)
{
	return channel_tech_caps;
}
